/**
 * jquery.contextmenu.js jQuery Plugin for Context Menus
 * http://www.JavascriptToolbox.com/lib/contextmenu/
 * 
 * Copyright (c) 2008 Matt Kruse (javascripttoolbox.com) Dual licensed under the
 * MIT and GPL licenses.
 * 
 * @version 1.1
 * @history 1.1 2010-01-25 Fixed a problem with 1.4 which caused undesired
...
(function($) {
	$.contextMenu = {
		...
		separator : 'context-menu-separator', // A specific key to identify a
												// separator
		target : null, // The target of the context click, to be populated when
						// triggered
		menu : null, // The jQuery object containing the HTML object that is
						// the menu itself
		shadowObj : null, // Shadow object
		bgiframe : null, // The iframe object for IE6
		shown : false, // Currently being shown?
		useIframe : /* @cc_on @ *//* @if (@_win32) true, @else @ */false,/* @end @ */// This
																				// is a
																				// better
																				// check
																				// than
		...
		create : function(menu, opts) {
			...
			// If a function has been passed in, call it each time the menu is
			// shown to create the menu
		...
		createMenuItem : function(label, obj) {
			...
			} // If passed a simple function, turn it into a property of an
				// object
		...
		// each time the item is hovered or clicked because the disabled status
		...
		// A hook to call before the menu is shown, in case special processing
		// needs to be done.
		...
		show : function(t, e) {
			...
			if (cmenu.beforeShow() !== false) {
				// If the menu content is a function, call it to populate the
				// menu each time it is displayed
		...
		// Find the position where the menu should appear, given an x,y of the
		// click event
