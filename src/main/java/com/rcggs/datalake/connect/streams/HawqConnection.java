...
public class HawqConnection extends AbstractDataLakeConnection implements
		DataLakeConnection {
	...
	public HawqConnection(ConnectionConfig config) {
		super(config);
		try {
			this.config = config;
		} catch (Exception e) {
...
//		schema.put("tweet", "json string");
		...
		try {
			...
			cd.setName("db0 - <font color=red>offline</font>");
			cd.setData(data);
			cd.setLeaf(true);
		//	cd.setSchema(schema);
